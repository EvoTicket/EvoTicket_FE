import { SEAT_ZONES } from "@/src/features/organizer/fixtures/eventDetail";
import { OrganizerStatusBadge } from "../common/OrganizerStatusBadge";

export function SeatMapPreview() {
  return (
    <div className="rounded-ds-lg border border-border-subtle bg-bg-elevated p-4">
      ...
      <div className="grid gap-3">
        {SEAT_ZONES.map((zone, index) => (
          <div
            key={zone.zone}
            className="grid grid-cols-[120px_minmax(0,1fr)_auto] items-center gap-3 rounded-ds-md border border-border-subtle bg-bg-surface p-3"
          >
